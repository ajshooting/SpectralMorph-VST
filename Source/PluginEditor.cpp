    ...
    : AudioProcessorEditor (&p), audioProcessor (p), visualizer(p)
{
    ...
    overallScaleSlider.setSliderStyle(juce::Slider::RotaryHorizontalVerticalDrag);
    overallScaleSlider.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 50, 20);
    addAndMakeVisible(overallScaleSlider);
    ...
    scaleAttachment = std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
        audioProcessor.getAPVTS(), "OVERALL_SCALE", overallScaleSlider);
...
void SpectralFormantMorpherAudioProcessorEditor::resized()
{
    ...
    overallScaleSlider.setBounds(bottomArea.removeFromLeft(100));
