...
SpectralProcessor::SpectralProcessor()
{
    ...
    extractedEnvelope.resize(numBins);
    warpedEnvelope.resize(numBins);

    visSpectrum.resize(numBins);
    visEnvelope.resize(numBins);
}
...
void SpectralProcessor::processBlock(std::vector<float>& data)
{
    ...
    for (int i = 0; i < numBins; ++i)
    {
        float originalEnv = std::max(extractedEnvelope[i], 1e-9f);
        float scale = warpedEnvelope[i] / originalEnv;

        fftBuffer[i * 2] *= scale;
        fftBuffer[i * 2 + 1] *= scale;
    }
    ...
    // Copy back
    for(int i=0; i<fftSize; ++i) data[i] = fftBuffer[i];
}
