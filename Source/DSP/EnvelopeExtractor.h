...
public:
    ...
    void prepare(int fftSize)
    {
        ...
        // Buffers
        timeDomainBuffer.resize(fftSize * 2);
        frequencyDomainBuffer.resize(fftSize * 2);
    }
    ...
    void process(const std::vector<float>& magnitudeSpectrum, std::vector<float>& envelope, int cutoffBin = 20)
    {
        ...
        {
            // Avoid log(0)
            float mag = std::max(magnitudeSpectrum[i], 1e-9f);
            float logMag = std::log(mag);

            // Set Real part
            frequencyDomainBuffer[i * 2] = logMag;
            // Imag part is 0
            frequencyDomainBuffer[i * 2 + 1] = 0.0f;
        }
        ...
        for (int i = 0; i < halfN; ++i)
        {
            ...
            float logEnv = frequencyDomainBuffer[i * 2];
            envelope[i] = std::exp(logEnv);
        }
    }
